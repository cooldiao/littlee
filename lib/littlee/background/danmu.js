var Danmu = {
	...
	init: function(){
		setTimeout(function(){
			$('body').append('<div class="danmu"></div>');
			$('body').append(template('danmu', {window: window}));
			$('.danmu-btn').bind('click', function(){
				$('.danmu-input-group,.danmu-switch').toggleClass('hide');
				$('.danmu-input').val('');
			});
			function send(){
				if(Danmu.insend){
					Tip.tip({content: '请勿频繁发送弹幕，谢谢~'});
					return false;
				}
				Danmu.insend = true;
				setTimeout(function(){
				...
				if(msg == ''){
					Tip.failure({content: '请先填写弹幕内容'});
				}else{
					$('.danmu-input').val('');
					Danmu.send(msg);
				}
			}
			$('.danmu-click').bind('click', send);
			$('.danmu-input').bind('keydown', function(e){
				if(e.which == 13){
					send();
				}
			});
			$('.danmu-switch').bind('click', function(){
				if(!Danmu.can_switch){
					Tip.tip({content: '操作太频繁，请稍候重试~'});
					return false;
				}
				Danmu.can_switch = false;
				setTimeout(function(){
					Danmu.can_switch = true;
				}, 1000);
				$('.danmu-switch').find('.mui-switch').toggleClass('mui-active');
				Danmu._stop = !Danmu._stop;
			});
			Danmu.deamon = setInterval(Danmu.close, 1000);
		}, 2000);
	},
	remove: function(){
		$('.danmu>*').each(function(i,n){
			n = $(n);
			if(n.css('bottom') == '2000px'){
				n.remove();
			}
		});
	},
	send: function(content){
		if(Danmu._stop || !Danmu._run) return false;
		content = content.substr(0, 50);
		content = Danmu.filter(content);
		content = JSON.stringify({"controller_name":"Account","method_name":"message", "message": content});
		Danmu.Socket.send(content);
	},
	filter: function(content){
		for(var i in Danmu.words){
			content = content.replace(new RegExp(Danmu.words[i], 'gi'), '*');
		}
		return content;
	},
	show: function(content){
		var color = Danmu.color[Math.floor(Math.random()*Danmu.color.length)];
		style = '-moz-animation: danmu 12s linear;color:'+color+';-webkit-animation: danmu 12s linear;animation: danmu 12s linear;';
		$('.danmu').append('<div style="'+style+'">'+content+'</div>');
	},
	close: function(){
		if(Danmu._stop == false){
			if(!Danmu._run){
				Danmu.start();
				Danmu._run = true;
			}
			...
			if(Danmu._run){
				Danmu.msg = [];
				$('.danmu').html('');
				clearInterval(Danmu._clear);
				try{
					Danmu.Socket.close();
				}catch(e){}
				clearInterval(Danmu._send);
				Danmu._run = false;
			}
		}
	},
	connect: function(){
		...
		Danmu.Socket = new WebSocket("ws://180.76.128.52:8083");
		Danmu.Socket.onopen = function (event) {
		    Danmu.Socket.send(JSON.stringify({ "controller_name": "Account", "method_name": "connect" }));
		    $('.danmu-switch').find('.mui-switch').addClass('mui-active');
		    Tip.tip({content: '弹幕服务器已连接'});
		};
		Danmu.Socket.onmessage = function(evt){
			var data = JSON.parse(evt.data);
			if(data.type == 'message'){
				Danmu.msg.push(data.message);
			}
		}
		Danmu.Socket.onclose = function(evt){
			if(!Danmu._stop){
				Danmu.connect();
			}else{
				Tip.tip({content: '弹幕已关闭'});
			}
		}
		Danmu.Socket.onerror = function(evt){
			Tip.tip({content: '弹幕服务器异常'});
			Danmu.connect();
		}
	},
	start: function(){
		Danmu._clear = setInterval(Danmu.remove, 2000);
		Danmu.connect();
		Danmu._send = setInterval(function(){
			if(Danmu.msg.length > 0){
				Danmu.show(Danmu.msg.pop());
			}
		},200);
	},
	destroy: function(){
		...
		Danmu.Socket.close();
		Danmu.msg = [];
		clearInterval(Danmu.deamon);
		$('.danmu').html('');
	}
};
background.Danmu = Danmu;
Danmu.init();